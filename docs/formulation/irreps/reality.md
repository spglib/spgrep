...
1. $\\Gamma$ is pseudo-real: $\\Gamma$ and $\\Gamma^{\\ast}$ are equivalent but can be taken as real matrices.
1. $\\Gamma$ is not equivalent to $\\Gamma^{\\ast}$.

These cases are classified with Frobenius-Schur indicator:
$$
\\frac{1}{|G|} \\sum\_{ g \\in G } \\chi(g^{2})
= \\begin{cases}
...
$$
\\frac{1}{|\\mathcal{G}^{\\mathbf{k}}|} \\sum\_{ g \\in \\mathcal{G}^{\\mathbf{k}} } \\chi^{\\mathbf{k}}(g^{2})
&= \\frac{1}{N} \\sum\_{ \\mathbf{t} } \\sum\_{ { i \\mid \\mathbf{S}_{i} \\in \\overline{\\mathcal{G}}^{\\mathbf{k}} } }
\\chi^{\\mathbf{k}}\\left( (\\mathbf{E}, \\mathbf{t})(\\mathbf{S}_{i}, \\mathbf{w}_{i}) (\\mathbf{E}, \\mathbf{t})(\\mathbf{S}_{i}, \\mathbf{w}_{i}) \\right) \\
&= \\frac{1}{N} \\sum_{ \\mathbf{t} } \\sum\_{ { i \\mid \\mathbf{S}_{i} \\in \\overline{\\mathcal{G}}^{\\mathbf{k}} } }
\\chi^{\\mathbf{k}}\\left( (\\mathbf{E}, \\mathbf{t} + \\mathbf{S}_{i}\\mathbf{t})(\\mathbf{S}_{i}, \\mathbf{w}_{i})^{2} \\right) \\
&= \\left(
\\frac{1}{N} \\sum\_{ \\mathbf{t} } e^{-i \\mathbf{k} \\cdot (\\mathbf{t} + \\mathbf{S}_{i}\\mathbf{t}) }
\\right)
\\left(
...
\\right)
\\sum\_{ { i \\mid \\mathbf{S}_{i} \\in \\overline{\\mathcal{G}}^{\\mathbf{k}} } }
