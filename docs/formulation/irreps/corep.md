...
$$
a (c\_{1} \\psi\_{1} + c\_{2} \\psi\_{2}) = c\_{1}^{\\ast} a\\psi\_{1} + c\_{2}^{\\ast} a\\psi\_{2}
$$
for $c\_{1}, c\_{2} \\in \\mathbb{C}$ and $\\psi\_{1}, \\psi\_{2} \\in V$.

## Definition

Let $M$ be magnetic point group other than type I.
Let $D$ be a maximal point subgroup of $M$.
Then, $M$ can be decomposed as $M = D \\sqcup a\_{0} D$, where $a\_{0}$ is an antisymmetry operation.
...
$$
where $u, u' \\in D$ and $a, a' \\in a\_{0}D$.
...
$$
for $u \\in D$, $a \\in a\_{0}D$, and $m, m \\in M$ [^footnote1].

\[^footnote1\]: Consider $\\mathbf{\\Gamma}(a) \\mathbf{\\Gamma}(m)^{\\ast} \\mathbf{\\Gamma}(m')^{\\ast}$ for $m \\in D$ and $\\mathbf{\\Gamma}(a) \\mathbf{\\Gamma}(m)^{\\ast} \\mathbf{\\Gamma}(m')$ for $m \\in a\_{0}D$.
...
\\mathbf{\\Gamma}'(a) &= \\mathbf{T}^{-1} \\mathbf{\\Gamma}(a) \\mathbf{T}^{\\ast} \\quad (a \\in a\_{0}D) \\
$$
...
&:= \\frac{ \\omega(u, a\_{0}) }{ \\omega( a\_{0}, a\_{0}^{-1} u a\_{0} ) } \\mathbf{\\Gamma}( a\_{0}^{-1} u a\_{0} )^{\\ast} \\quad (u \\in D) \\
u a\_{0} \\mathbf{\\phi}_{j} &= \\sum_{j} a\_{0} \\mathbf{\\phi}_{j} \\overline{\\mathbf{\\Gamma}}(u)_{ij}.
$$

The following Frobenius-Schur indicator should be one of ${ -1, 0, 1 }$:
$$
\\xi^{\\alpha} &:= \\frac{1}{|D|} \\sum\_{ u \\in D } \\omega(a\_{0}u, a\_{0}u) \\chi( (a\_{0}u)^{2} ) \\
...
$$
Then, a transformed basis ${ \\mathbf{\\psi}_{i} := \\sum_{j} a\_{0} \\mathbf{\\phi}_{j} [\\mathbf{U}^{\\dagger}]_{ji} }\_{i}$ also forms $\\Gamma$.

A new basis ${ \\frac{1}{\\sqrt{2}}(\\phi\_{i} + \\psi\_{i}) }_{i}$ gives the following irrep \[^derivation\]:
$$
\\tilde{\\mathbf{\\Gamma}}(u) &= \\mathbf{\\Gamma}(u) \\
\\tilde{\\mathbf{\\Gamma}}(a_{0}) &= \\mathbf{U} \\
\\tilde{\\mathbf{\\Gamma}}(a\_{0}u) &= \\omega(a\_{0}, u)^{\\ast} \\tilde{\\mathbf{\\Gamma}}(a\_{0}) \\tilde{\\mathbf{\\Gamma}}(u) \\
\\mathbf{U}\\mathbf{U}^{\\ast} &= \\omega(a\_{0}, a\_{0})\\Gamma(a\_{0}^{2}).
$$

\[^derivation\]: Be careful anti-linear operators,
$$
a\_{0} \\psi\_{i}
&= \\sum\_{j} [\\mathbf{U}^{\\dagger\\ast}]_{ji} a_{0} (a\_{0} \\phi\_{j}) \\
&= \\sum\_{jk} \\phi\_{k} \\omega(a\_{0}, a\_{0}) \\Gamma(a\_{0}^{2})_{kj} [\\mathbf{U}^{\\dagger\\ast}]_{ji} \\
&= \\sum\_{k} \\phi\_{k} U\_{ki}
$$
...
We can take $(\\mathbf{\\phi}_{1}, \\cdots, \\mathbf{\\phi}_{d}, a\_{0}\\mathbf{\\phi}_{1}, \\cdots, a_{0}\\mathbf{\\phi}_{d})$ as basis and they form irrep of $M$ as
$$
\\tilde{\\mathbf{\\Gamma}}(u) &=
\\begin{pmatrix}
\\mathbf{\\Gamma}(u) & \\mathbf{0} \\
...
\\begin{pmatrix}
\\mathbf{0} & \\omega(a\_{0}, a\_{0}) \\mathbf{\\Gamma}(a\_{0}^{2}) \\
\\mathbf{1} & \\mathbf{0} \\
\\end{pmatrix} \\
\\tilde{\\mathbf{\\Gamma}}(a\_{0}u) &= \\omega(a\_{0}, u)^{\\ast} \\tilde{\\mathbf{\\Gamma}}(a\_{0}) \\tilde{\\mathbf{\\Gamma}}(u)
$$
...
Let $M$ be magnetic point group other than type I.
Let $D$ be a maximal point subgroup of $M$.
Then, $M$ can be decomposed as $M = D \\sqcup D a\_{0}$, where $a\_{0} = \\mathbf{S}_{0} 1'$ is an antisymmetry operation.
...
D a_{0} \\ni \\mathbf{S}1' &\\mapsto \\mathbf{U}(\\mathbf{S}) \\left( -i \\sigma\_{y} \\right) K,
% a\_{0} &\\mapsto \\mathbf{U}(\\mathbf{S}_{0}) \\left( -i \\sigma_{y} \\right) K \\
% a\_{0} D \\ni a\_{0} \\mathbf{S} &\\mapsto \\mathbf{U}(\\mathbf{S}_{0}) \\left( -i \\sigma_{y} \\right) \\mathbf{U}(\\mathbf{S})^{\\ast} K,
$$
where $\\mathbf{U}: \\mathrm{O}(3) \\to \\mathrm{SU}(2)$ is defined {ref}`here <spin_representation>`.
$\\sigma\_{y}$ is the Pauli matrix.
