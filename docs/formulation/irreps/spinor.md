...
\\Rightarrow \\left[ g\_{1} \\left[ g\_{2} \\mathbf{\\Psi} \\right] \\right](%5Cmathbf%7Br%7D)
&= \\mathbf{U}(\\mathbf{p}_{g_{1}}) \\left[ g\_{2} \\mathbf{\\Psi} \\right](g_%7B1%7D%5E%7B-1%7D%5Cmathbf%7Br%7D) \\
&= z(\\mathbf{p}_{g_{1}}, \\mathbf{p}_{g_{2}}) \\left[ (g\_{1}g\_{2}) \\mathbf{\\Psi} \\right](%5Cmathbf%7Br%7D).
$$
...
Let {math}`\Gamma^{\mathbf{k}\alpha}` be a projective irrep of the little group {math}`\mathcal{G}^{\mathbf{k}} = \coprod_{ \{ i \mid \mathbf{S}_{i} \in \overline{\mathcal{G}}^{\mathbf{k}} \} } (\mathbf{S}_{i}, \mathbf{w}_{i}) \mathcal{T}` and ${ \\mathbf{\\Psi}^{\\mathbf{k}\\alpha}_{\\mu} }_{\\mu=1}^{d\_{\\mathbf{k}\\alpha}}$ form the projective irrep.
...
&=
\\sum\_{\\mu} \\mathbf{\\Psi}^{\\mathbf{k}\\alpha}_{\\mu}
...
We can enumerate projective irreps for spinor from the factor system $\\mu\_{\\mathbf{k}}(S\_{i}, S\_{j}) := z(\\mathbf{S}_{i}, \\mathbf{S}_{j}) \\exp \\left( -i \\mathbf{g}_{i} \\cdot \\mathbf{w}_{j} \\right)$.
...
&= \\begin{pmatrix}
\\cos \\theta + n\_{x}^{2} (1 - \\cos \\theta) & n\_{x} n\_{y} (1 - \\cos \\theta) - n\_{z} \\sin \\theta & n\_{x} n\_{z} (1 - \\cos \\theta) + n\_{y} \\sin \\theta \\
n\_{y} n\_{x} (1 - \\cos \\theta) + n\_{z} \\sin \\theta & \\cos \\theta + n\_{y}^{2} (1 - \\cos \\theta) & n\_{y} n\_{z} (1 - \\cos \\theta) - n\_{x} \\sin \\theta \\
n\_{z} n\_{x} (1 - \\cos \\theta) - n\_{y} \\sin \\theta & n\_{z} n\_{y} (1 - \\cos \\theta) + n\_{x} \\sin \\theta & \\cos \\theta + n\_{z}^{2} (1 - \\cos \\theta) \\
\\end{pmatrix} \\
\\mathbf{L}_{x}
&:= -i \\begin{pmatrix}
0 & 0 & 0 \\
...
$$
\\begin{pmatrix}
[R\_{ \\theta\\hat{\\mathbf{n}} }]_{23} - \[R_{ \\theta\\hat{\\mathbf{n}} }\]_{32} \\
...
&=
\\begin{pmatrix}
\\cos \\frac{\\theta}{2} - i n_{z} \\sin \\frac{\\theta}{2} & (-i n\_{x} - n\_{y}) \\sin \\frac{\\theta}{2} \\
(-i n\_{x} + n\_{y}) \\sin \\frac{\\theta}{2} & \\cos \\frac{\\theta}{2} + i n\_{z} \\sin \\frac{\\theta}{2} \\
\\end{pmatrix}.
$$
